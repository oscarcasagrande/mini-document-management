...
Estado em 2026-09-25. Este arquivo é para quem retomar o trabalho, com ou sem contexto da conversa.
A fonte de verdade de produto continua sendo `docs/PRD.md`; as convenções, `CLAUDE.md`.

## Situação em uma frase

**Atualização de 2026-09-25:** o espaço em disco foi liberado, o Docker voltou e a Etapa 2 foi
compilada, testada e aceita via `docker compose` (resultado na seção "Resultado do aceite", ao fim).
O restante deste arquivo é o registro do que foi escrito e decidido enquanto o ambiente estava
bloqueado; as listas de pendência abaixo já foram executadas, exceto onde dito o contrário.

O bloqueio original foi ambiental: o disco `C:` chegou a 0 bytes livres, o disco virtual do Docker
Desktop (`docker_data.vhdx`, ~42,5 GB) entrou em somente leitura, e sem Docker não há build .NET,
testes, compose nem aceite (`scripts/dotnet.sh` também roda em contêiner).
...
4. `GET /text` devolve o texto bruto por página; `GET /result` devolve os campos do `BR_CPF_CARD`
   ...
   `DOC-20260924-000003` é o **cartão enviado pela UI**, não a amostra sintética. Esperado para ele:
   CPF `332.200.116-44` com `validationStatus` `INVALID` (o segundo DV correto seria `0`),
   nome `DUDA CID WELZHU`, nascimento `1991-01-01`.
   Esperado para o teste com a amostra sintética `cpf-card-limpo` (enviada como documento novo):
   CPF `11144477735` (`VALID`), nome `MARIA APARECIDA DA SILVA SOUZA`, nascimento `1985-03-14`.
...
## Resultado do aceite (2026-09-25)

Executado sobre imagens construídas por `docker compose up -d --build`, com o código final.

1. **Testes:** .NET unit 235/235; integração 21/21 contra o Postgres do compose (nenhum pulado);
   `pytest` do ocr-service 19/19; `tsc --noEmit` limpo; `dotnet build` com 0 warnings.
2. **Compose:** cinco serviços healthy; `ocr-service` com `HostConfig.Memory` = 3221225472 (3 GiB).
3. **`DOC-20260924-000003` (cartão enviado pela UI):** reprocessado, `QUEUED` → `PREPROCESSING` →
   `OCR_RUNNING` → `CLASSIFYING` → `EXTRACTING` → `COMPLETED` (linha do tempo nos eventos).
4. **`/text` e `/result` e tela de detalhe:** cpf `332.200.116-44` `INVALID` (`CHECK_DIGIT_INVALID`),
   nome `DUDA CID WELZHU` `VALID`, nascimento `1991-01-01` `VALID`. Amostra sintética `cpf-card-limpo`
   enviada como documento novo: `11144477735` `VALID`, `MARIA APARECIDA DA SILVA SOUZA`, `1985-03-14`.
5. **`ocr-service` derrubado no meio:** `/content` respondeu 200; documento voltou a `QUEUED` com
   `OCR_UNAVAILABLE` e `nextAttemptAt`; com o serviço parado, 3 tentativas (15 s e 30 s de backoff)
   terminaram em `FAILED`; `POST /reprocess` depois disso levou a `COMPLETED`; numa segunda queda, com o
   serviço de volta dentro do prazo, o documento chegou a `COMPLETED` sozinho na tentativa 2.
6. **Logs** de api, worker, ocr-service, web-bff e postgres: nenhuma ocorrência de CPF, nome, datas,
   texto do cartão ou nome de arquivo.

Defeitos encontrados e corrigidos no caminho (todos com teste):
- `PaddleOcrServiceProvider` fechava o stream do original após a primeira página (falharia em todo PDF
  multipágina); agora bufferiza os bytes uma vez.
- `DocumentEvent.Id` sem `ValueGeneratedNever` fazia o EF emitir `UPDATE` em vez de `INSERT` para evento
  novo de documento rastreado (quebrava completar, reprocessar, falhar e liberar job).
- `NO_LABEL_NEARBY` aparecia em CPF inválido que estava ao lado do rótulo; a confiança desse caso agora
  inclui a penalidade de distância ao rótulo (0,70 → 0,65 no cartão acima).
- Worker logava todo comando SQL do EF em Information; alinhado ao da API (Warning).
- `Dockerfile.worker` não copiava `src/`; agora copia como o da API.
- Comando de migration: `--startup-project` passou a ser `src/DocReader.Infrastructure` (o projeto da API
  não referencia `Microsoft.EntityFrameworkCore.Design`).

Observações que ficam abertas:
- `expectedDocumentType` é texto livre; "CPF" digitado no upload aparece como `divergent=true` frente a
  `BR_CPF_CARD`, como o RF-010 manda (registrar, não corrigir).
- A pendência de latência em A4 continua (ADR 0002).
